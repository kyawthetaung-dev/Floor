    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `note` (`id` TEXT NOT NULL, `title` TEXT NOT NULL, `message` TEXT NOT NULL, PRIMARY KEY (`id`))');
...
class _$NoteDao extends NoteDao {
  ...
  Stream<List<Note>> getAllNote() {
    return _queryAdapter.queryListStream('select * from note',
        mapper: (Map<String, Object?> row) => Note(row['id'] as String,
  ...
  @override
  Future<List<Note>> findAllNote() async {
  ...
  @override
  Future<void> insertAllNote(List<Note> notes) async {
  ...
  @override
  Future<void> deleteAllNote(List<Note> noteList) async {
    await _noteDeletionAdapter.deleteList(noteList);
  }
}
